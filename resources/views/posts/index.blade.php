        ...
        <div class="space-y-10 md:space-y-16">
            ...
            @foreach($posts as $post)
                ...
                <article class="flex flex-col lg:flex-row pb-10 md:pb-16 border-b">
                    <div class="lg:w-5/12">
                        <img class="w-full max-h-72 object-cover lg:max-h-none lg:h-full" src="{{$post->thumbnail}}" alt="{{$post->id}}">
                    </div>
                    <div class="flex flex-col items-start mt-5 space-y-5 lg:w-7/12 lg:mt-0 lg:ml-12">
                        ...
                        <p class="text-slate text-lg">{{ $post->excerpt }}</p>
{{--                            {{ route('posts.show', ['post' => $post]) }}--}}
                        <a href="" class="text-indigo-700 font-semibold hover:underline">Lire la suite</a>

                        <div class="flex items-center space-x-2 text-slate-300 text-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M5.25 6.75h13.5M5.25 12h13.5m-13.5 5.25H12" />
                            </svg>
...
{{--                                        {{ route('posts.byTag', ['tag' => $tag]) }}--}}
{{--                                        <a href="" class="underline">{{ $tag->name }}</a>{{ !$loop->last ? ', ' : '' }}--}}
{{--                                    @endforeach--}}
{{--                                </span>--}}
{{--                            @endif--}}
